...
- **Быстрое чтение (Bionic)** - Переключение на шрифт, который выделяет начало слов для более комфортного чтения длинных документов
...
- Быстрое чтение через Bionic-подобный шрифт 🧠
...
На панели инструментов есть кнопка **FastFont**: она переключает текущий документ на встроенные Bionic-подобные шрифты. Плагин использует локальные `Fast Sans` и `Fast Mono`, поэтому режим не требует интернета и не переписывает исходный Markdown.

Идея такого шрифта в том, что первая часть каждого слова визуально выделяется и создаёт для глаз «точки фиксации». Короткое описание подхода: [создан бионический шрифт, ускоряющий процесс чтения](https://www.mentoday.ru/life/news/20-01-2025/sozdan-bionicheskii-shrift-uskoryayushchii-process-chteniya-kak-eto-rabotaet/).
